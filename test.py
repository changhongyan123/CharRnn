...
pool_length = 2
# document input
document = Input(shape=(max_sentences, maxlen), dtype='int64')#matrix
# sentence input
in_sentence = Input(shape=(maxlen,), dtype='int64')#1D,contain the character of one stence.
...
for i in range(len(nb_filter)):#??????????????
    ...
    embedded = Dropout(0.1)(embedded)#prvent overfitting
...
if checkpoint:
    model.load_weights(checkpoint)#load weight from hdf5 file.
